---
description: Create a branch or pull request for this ar-002-clubs repo when the user gives a TU task, Notion spec link, task DB link, or GitHub PR link. Use it whenever work needs a standardized TU branch or dev-targeted PR. Confirm the task document first, create a Notion task page in the task DB when one does not exist, extract the TU number, then create or reuse the correct branch and open a correctly titled PR with Notion context attached.
---
