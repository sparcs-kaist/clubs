...
- PR 생성 헬퍼나 `--body-file`을 사용할 때도 이 section/block을 반드시 포함하고 검증합니다.
...
- 백엔드 production source에서 복합 조건문, boolean decision, array predicate, `ts-pattern`의 `P.when`/`Pattern.when` 조건을 새로 추가하거나 수정하면 MC/DC 검증 대상입니다.
- 조건식이 서비스/리포지토리 의존성과 섞여 있으면, 가능한 한 순수 함수로 분리하고 해당 함수의 Jest unit test로 각 원자 조건이 독립적으로 결과를 바꾸는 케이스를 작성합니다.
- push 전에는 Husky `pre-push` 훅이 `pnpm mcdc:changed --fail-on-missing`을 실행합니다. 이 명령은 `origin/dev` 대비 변경된 production source의 신규/수정 복합 decision만 검사합니다.
- MC/DC runtime instrumentation은 JavaScript short-circuit 의미를 바꾸지 않도록 함수 호출, property access, element access가 포함된 원자 조건을 보수적으로 제외합니다. 이런 조건은 필요한 값을 지역 변수로 먼저 추출한 뒤 boolean 조합을 decision으로 드러내는 방식으로 작성합니다.
- 긴급하게 훅을 건너뛰어야 할 때는 `HUSKY=0 git push`를 사용하되, PR 전에 `pnpm mcdc:changed --fail-on-missing`을 별도로 통과시켜야 합니다.
